...
def _render_current_fen(operator, context):
    ...
    if (
        any(l.get("type") == "adjacencies" and l.get("enabled") for l in layers)
        and adjacencies_data is None
    ):
    ...
    if (
        any(l.get("type") == "links" and l.get("enabled") for l in layers)
        and links_data is None
    ):
        operator.report({"INFO"}, "Fetching links for links layer...")
        links_data = connector_service.fetch_links(props.fen_string)

    if (
        any(l.get("type") == "king_box" and l.get("enabled") for l in layers)
        and king_box_data is None
    ):
        operator.report({"INFO"}, "Fetching king_box for king_box layer...")
        king_box_data = connector_service.fetch_king_box(props.fen_string)

    if (
        any(l.get("type") == "shadows" and l.get("enabled") for l in layers)
        and shadows_data is None
    ):
    ...
    operator.report(
        {"INFO"},
        f"Rendered {len(data['nodes'])} pieces across {len([l for l in layers if l.get('enabled')])} layers",
    )
    return True, data
...
def _trigger_animation(
    ...
    try:
        ...
        logger.error("_trigger_animation failed", exc_info=True)
...
class BLCHESS_OT_undo_move(Operator):
    ...
    def execute(self, context):
        ...
        _trigger_animation(
            from_fen, props.fen_string, connector_url=props.connector_url
        )
        return {"FINISHED"}
