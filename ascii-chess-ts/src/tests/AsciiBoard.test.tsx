...
test('changes FEN on input change', () => {
  ...
  const fenInput = screen.getByLabelText('FEN Input');
  fireEvent.change(fenInput, { target: { value: 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1' } });

  expect(fenInput.value).toBe('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1');
});
...
test('submits move on button click', () => {
  ...
  const moveInput = screen.getByLabelText('Move Input');
